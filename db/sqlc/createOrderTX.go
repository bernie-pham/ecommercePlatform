...
func (store *SQLStore) CreateOrderTX_v2(ctx context.Context, arg CreateOrderTXParams) error {
	...
	err := store.execTX(ctx, func(q *Queries) error {
		...
		for _, item := range arg.Items {
			...
			total_price = float32(item.Quantity) * price
			...
			if product_entry.DealID.Valid {
				...
				if discount > float32(deal.DealLimit.Float64) {
					discount = float32(deal.DealLimit.Float64)
				}
			...
			orderMap[int32(merchantID)] = append(orderMap[int32(merchantID)], orderItemParam)
			// if _, ok := orderMap[merchantID]; ok {

			// } else {
			// 	orderMap[merchantID] = []CreateOrderItemParams{orderItemParam}
			// }
			err = q.DeleteCartItemByID(ctx, DeleteCartItemByIDParams{
				ID:     item.ID,
				UserID: item.UserID,
			})
			if err != nil {
		...
		if dealValid {
			...
			if orderDiscount > float32(deal.DealLimit.Float64) {
				orderDiscount = float32(deal.DealLimit.Float64)
			}
